...
def build(arch, build):
    ...
    subprocess.call("git apply --ignore-whitespace ../patches/probeoverflowfix.patch - v", cwd=getSrcPath(), shell=True)
    subprocess.call("git apply --ignore-whitespace ../patches/disableframedropping.patch - v", cwd=getSrcPath(), shell=True)
    subprocess.call("git apply --ignore-whitespace ../patches/disableaudioprocessing.patch - v", cwd=getSrcPath(), shell=True)
    subprocess.call("git apply --ignore-whitespace ../patches/exposejitter.patch -v", cwd=getSrcPath(), shell=True)
    subprocess.call("git apply --ignore-whitespace ../patches/disablepacing.patch -v", cwd=getSrcPath(), shell=True)
    subprocess.call("git apply --ignore-whitespace ../patches/fasterprobing.patch -v", cwd=getSrcPath(), shell=True)
